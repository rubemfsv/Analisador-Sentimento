...
basetreinamento = [
    ...
    (
        "o miserável receio de ser sentimental e o mais vil de todos os receios modernos",
        "desgosto",
    ),
    ...
    (
        "não sei como e a vida de um patife, mais a de um homem honesto e abominável",
        "desgosto",
    ),
    (
        "há coisas que temos que suportar para não acharmos a vida insuportável",
        "desgosto",
    ),
...
def aplicastemmer(texto):
    ...
    for (palavras, emocao) in texto:
        withstemming = [
            str(stemmer.stem(p)) for p in palavras.split() if p not in stopwordsnltk
        ]
...
)  # Monta a tabela base de dados - classe
...
print(
    "Bem vindo ao Sistema de Inferência de Emoções, digite uma frase abaixo que inferiremos qual emoção está contida, provavelmente nessa frase. Se quiser sair da aplicação, digite sair"
)
...
while 1:
    ...
    )  # Mostra a label da emoção que a frase é
    print("\n")
